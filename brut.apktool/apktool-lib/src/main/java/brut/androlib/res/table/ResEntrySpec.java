...
public class ResEntrySpec {
    ...
    static boolean isValidEntryName(String name) {
        ...
        // Must start with a valid Java identifier start character, but not '$' (rejected by aapt2).
        char first = name.charAt(0);
        if (!Character.isJavaIdentifierStart(first) || first == '$') {
            return false;
        }
        // The rest must be valid Java identifier part characters (excluding '$') or any of the whitelisted
        // special characters.
        for (int i = 1; i < len; i++) {
